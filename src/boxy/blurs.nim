...
## when not defined(ds3). Blur effects are not implemented on 3DS — PICA200
...
when not defined(ds3):
  ...
  # bounds, so the uniform-bounded loops above are illegal there. These use a constant
  # MaxBlurRadius bound + early `break` (and round()->floor(x+0.5), since round is ES
  # 3.00+). They sample x=0 once and ±x symmetrically — identical taps/weights to the
  # [-r, r] loop. Selected only under -d:vita in newBoxy.
