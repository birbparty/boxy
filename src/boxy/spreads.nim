...
# GLSL ES 1.00 (glslES1 / Vita) variants. Like blur, the uniform-bounded loops above are
# illegal in GLSL ES 1.00 (constant bounds required). NOTE: unlike blur, shady's glslES1
# does NOT reject these — it silently emits a uniform-bounded `for`, which vitaGL's
# SceShaccCg then crashes linking (confirmed on hardware). So these constant-bound +
# `break` variants are mandatory for spread on Vita. Symmetric (0, ±x) = same [-r, r] window.
