...
##
## Filter/Wrap types are always available (GL-free, from backend_interface).
## Everything else — Texture type, toGLenum converters, all GL procs — is
## guarded under `when not defined(ds3)`. On 3DS, texture management is
## handled via TextureHandle through the citro3d backend.

import backends/backend_interface

# Re-export Filter and Wrap so existing callers (boxy.nim) don't need a
# new direct import of backend_interface.
export Filter, Wrap
...
when not defined(ds3):
  ...
  # The filterDefault / wDefault arms are never reached — call sites guard
  # with != filterDefault / != wDefault before invoking glTexParameteri.
  # ---------------------------------------------------------------------------
