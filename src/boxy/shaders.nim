...
## shader is assembled inline at Nim-compile time via Shady toPicaShbin
## (src/boxy/backends/render2d_pica.nim) and used by citro3d_backend; this
## module is not compiled for ds3.
      ...
      if name == attrib.name:
        if buffer.normalized or buffer.kind != bkSCALAR:
          ...
          when defined(vita):
            ...
            # all float/normalized, so this integer-attribute branch is never taken —
            # guard it out so the symbol isn't referenced at link.
